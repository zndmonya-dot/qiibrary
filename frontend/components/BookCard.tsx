...
function BookCard({ rank, book, stats, topArticles, onNavigate }: BookCardProps) {
  // ランクに応じたバッジのスタイル（統一デザイン）
  const getRankStyle = () => {
    if (rank === 1) return 'bg-yellow-500 text-black border-yellow-300 shadow-[3px_3px_0_#a16207]';
    if (rank === 2) return 'bg-gray-400 text-black border-gray-200 shadow-[3px_3px_0_#4b5563]';
    if (rank === 3) return 'bg-amber-600 text-black border-amber-400 shadow-[3px_3px_0_#92400e]';
    return 'bg-gray-800 text-green-400 border-green-600 shadow-[2px_2px_0_#166534]';
  };
  ...
  return (
    <div className="bg-black border-2 border-gray-800 relative group hover:border-green-500 transition-all duration-300 shadow-[4px_4px_0_#1a1a1a] hover:shadow-[4px_4px_0_#166534]">
      {/* Rank Badge */}
      <div className={`absolute -top-4 left-4 font-pixel text-xs px-3 py-1.5 z-10 border-2 ${getRankStyle()}`}>
        {rank}
      </div>
